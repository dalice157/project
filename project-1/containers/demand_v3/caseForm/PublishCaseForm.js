...
import OtherCaseForm from '../../../components/demand_v3/caseForm/OtherCaseForm';
import { caseRoleTypes, moneyData } from '../../../config/selectData';
...
class PublishCaseForm extends PureComponent {
    ...
    onSubmit = async (values, actions) => {
      ...
      const {
        demandCategory, title, unit, minPrice, maxPrice, placeType, assignPlace, partnerCount, desc, name, email, sex, cellphone, tel, telArea, otherContactWay, experience, jobOccupation, contactTimeBegin, contactTimeEnd
      } = values;
      // 使用者是否手機號碼與區域號碼二擇一
  ...
  initializeData = (initialCaseForm, area, isNewDemandForm) => {
    ...
    // 新開案預設no, 載入案件由reducer判斷
    const newPlaceType = isNewDemandForm ? placeType : 'no';
    const initialData = {
      ...initialCaseForm,
      demandCategory: isDemandCatValid ? demandCategory : [],
      assignPlace: searchArea,
      placeType: newPlaceType,
      unit: isUnitExist ? moneyData[unit].value : moneyData[0].value,
