...
import {
  location, moneyData, targetData, defaultMoneyData, partnerCountData, sexTitle, demandExperienceData, tutorRoleTypes, tutorSexes, tutorFrequency, tutorFrequencyCount, tutorFrequencyHour, tutorFrequencyWeek, tutorTimeList, tutorGrade, planTypeList, teachTypes, teachDurationTypes, teachPlaceTypes, timeList
} from '../../../config/selectData';
import { tutorTreeData } from '../../../config/demandPublish';
import Button from '../../ui/button';
import styles from './EditTutorForm.scss';
...
class TutorForm extends PureComponent {
  ...
  renderForm = (props) => {
    ...
    const {
      onPrev, isModifiedDemand, planType, isSubmitting, isLoadingDemandForm
    } = this.props;
    const {
      hideCellphone, hideTelephone, hideOtherContactWay, demandCategory, unit, minPrice, maxPrice, placeType, educationalStage, desc, cellphone, tel, telArea, classEveryWeekHourBegin, classEveryWeekHourEnd, educationalGrade, classFrequencyHour, jobOccupation, contactTimeBegin, contactTimeEnd
    } = values;
      ...
      <form className={styles.form} onSubmit={handleSubmit}>
        ...
        <Spin tip="讀取聯絡人資料中" size="large" spinning={isLoadingDemandForm}>
                        ...
                        <Field
                          ...
                          render={({ field }) => (
                            <Checkbox
                              {...field}
                              id="hideCellphone"
                              checked={hideCellphone}
                              onChange={element => setFieldValue('hideCellphone', element.target.checked)}
                            >
                              不公開
                            </Checkbox>
                          )}
                        ...
                        <Field
                          ...
                          render={({ field }) => (
                            <Checkbox
                              {...field}
                              id="hideTelephone"
                              checked={hideTelephone}
                              onChange={element => setFieldValue('hideTelephone', element.target.checked)}
                            >
                              不公開
                            </Checkbox>
                          )}
          ...
          <div className={styles.field}>
            <span className={styles.fieldPublishTitle}>其他聯絡方式或備註說明(非必填)</span>
            {
                    planType === planTypeList[1].value
                      ? (
                        <Field
                          name="hideOtherContactWay"
                          render={({ field }) => (
                            <Checkbox
                              {...field}
                              id="hideOtherContactWay"
                              checked={hideOtherContactWay}
                              onChange={element => setFieldValue('hideOtherContactWay', element.target.checked)}
                            >
                              不公開
                            </Checkbox>
                          )}
          ...
          <div className={styles.btnWrap}>
            <Button onClick={() => handleReset(onPrev())}>取消編輯</Button>
            <Button onClick={() => alertSubmitForm(errors)} type="primary" htmlType="submit" loading={isSubmitting}>儲存編輯</Button>
          </div>
          {/* <DebugFormik /> */}
        </Spin>
      </form>
