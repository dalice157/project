...
import {
  tutorTimeList, location, moneyData, targetData, defaultMoneyData, partnerCountData, sexTitle, demandExperienceData, caseRoleTypes, planTypeList, timeList
} from '../../../config/selectData';
import { getContentByCat, caseTreeData } from '../../../config/demandPublish';
import Button from '../../ui/button';
import styles from './EditOtherCaseForm.scss';
...
class OtherCaseForm extends PureComponent {
  ...
  renderForm = (props) => {
    const {
      onPrev, isModifiedDemand, isLoadingDemandForm, planType, isSubmitting
    } = this.props;
    const {
      handleSubmit, handleReset, values, errors, setFieldValue, validateField
    } = props;
    const {
      hideCellphone, hideTelephone, hideOtherContactWay, demandCategory, unit, minPrice, maxPrice, placeType, desc, cellphone, tel, telArea, jobOccupation, contactTimeBegin, contactTimeEnd
    } = values;
