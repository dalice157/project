...
import DurationSearch from '../../../components/statistics/plans/DurationSearch';
import { loadPlanStatisticsTable } from '../../../actions/statistics';
import PlansTable from '../../../components/statistics/plans/PlansTable';
import PlansChart from '../../../components/statistics/plans/PlansChart';
...
class Plans extends PureComponent {
  ...
  async componentDidMount() {
    ...
    if (targetStartDate && targetEndDate && comparedStartDate && comparedEndDate) {
      ...
      await this.props.loadPlanStatisticsTable(dayjs(targetStartDate).format('YYYY/MM/DD'), dayjs(targetEndDate).format('YYYY/MM/DD'), dayjs(comparedStartDate).format('YYYY/MM/DD'), dayjs(comparedEndDate).format('YYYY/MM/DD'));
      this.setState({
        targetStartDate, targetEndDate, comparedStartDate, comparedEndDate,
      });
    } else if (targetStartDate && targetEndDate) {
      message.loading('報表資料載入中');
      await this.props.loadPlanStatisticsTable(dayjs(targetStartDate).format('YYYY/MM/DD'), dayjs(targetEndDate).format('YYYY/MM/DD'));
    ...
    return (
      <>
        <Breadcrumb style={{ margin: '16px 0' }}>
          ...
          <Breadcrumb.Item><h1 className="breadcrumb-header">經管付費方案統計表</h1></Breadcrumb.Item>
        </Breadcrumb>
