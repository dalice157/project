...
import TabGig from '../../components/member/memo/tabGig.js';
...
class MemoGig extends Component {
  state = {
    postNum: '',
    editing: false,
    loading: false,
  ...
  async componentDidMount() {
    ...
    if (basicId) {
      ...
      this.props.loadMemoList(basicId, '', 'gig').then(() => {
        this.setState({
          loading: false,
  ...
  loadMore = () => {
    ...
    this.props.loadMemoList(basicId, this.state.cursor, 'gig').then((res) => {
      ...
      <Spin tip="Loading..." spinning={ this.state.loading }>
        ...
        <div className="memoWrap">
          ...
          <div className="memoContent">
            ...
            />
            {
              items.length > 0 ? <TabGig data={ items } visible={ visible } /> : <div className="noData">無接案備註資料</div>
            }
            {
              cursor
              && (
...
export default connect(mapStateToProps, mapDispatchToProps)(MemoGig);
