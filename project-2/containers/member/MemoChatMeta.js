...
import TabChatMeta from '../../components/member/memo/tabChatMeta.js';
...
class MemoChatMeta extends Component {
  state = {
    postNum: '',
    editing: false,
    loading: false,
  ...
  async componentDidMount() {
    ...
    if (basicId) {
      ...
      this.props.loadMemoList(basicId, '', 'chatMeta').then(() => {
        this.setState({
          loading: false,
  ...
  loadMore = () => {
    ...
    this.props.loadMemoList(basicId, this.state.cursor, 'chatMeta').then((res) => {
      ...
      <Spin tip="Loading..." spinning={ this.state.loading }>
        ...
        <div className="memoWrap">
          ...
          <div className="memoContent">
            ...
            />
            {
              items.length > 0 ? <TabChatMeta data={ items } visible={ visible } /> : <div className="noData">無聊天室備註資料</div>
            }
            {
              cursor
              && (
...
export default connect(mapStateToProps, mapDispatchToProps)(MemoChatMeta);
