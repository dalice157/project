...
import TabViolation from '../../components/member/memo/tabViolation.js';
...
class MemoViolation extends Component {
  state = {
    postNum: '',
    editing: false,
    loading: false,
  ...
  async componentDidMount() {
    ...
    if (basicId) {
      ...
      this.props.loadMemoList(basicId, '', 'violation').then(() => {
        this.setState({
          loading: false,
  ...
  loadMore = () => {
    ...
    this.props.loadMemoList(basicId, this.state.cursor, 'violation').then((res) => {
      ...
      <Spin tip="Loading..." spinning={ this.state.loading }>
        ...
        <div className="memoWrap">
          ...
          <div className="memoContent">
            ...
            />
            {
              items.length > 0 ? <TabViolation data={ items } visible={ visible } /> : <div className="noData">無檢舉備註資料</div>
            }
            {
              cursor
              && (
...
export default connect(mapStateToProps, mapDispatchToProps)(MemoViolation);
