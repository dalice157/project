...
// 建立並取得檔案上傳簽章
const requestDocumentSignature = (param, basicId) => {
  return {
    [RSAA]: {
      endpoint: `${JAVA_URL}/bs/document/signature?basicId=${basicId}`,
      method: 'post',
      headers: {
...
const requestDocumentArrayUrl = (param) => {
  return {
    [RSAA]: {
      endpoint: `${JAVA_URL}/bs/document/file-url/${param.fileId}`,
      types: [
...
const requestDocumentReConvert = (param, basicId) => ({
  [RSAA]: {
    endpoint: `${JAVA_URL}/bs/document/reconvert/${param.fileId}?basicId=${basicId}`,
    types: [
...
export async function requestFileUrl(dispatch, newFileId, convertType) {
  ...
  fileUrlMap = fileUrls.reduce((urlsMap, f) => (f.tag ? Object.assign(urlsMap, { [f.tag]: f.url }) : urlsMap));

  return fileUrlMap;
}

const pollingUrlRequest = async (
  dispatch,
  newFileId,
  convertType,
  componentType,
  ...
  if (fileUrls.every(file => file.convertStatus === 'success')) {
    ...
    && fileUrls.every(
      file => file.convertStatus === 'pending' || file.convertStatus === 'uploading'
    )
  ) {
    passingTime += 2000;
    await timeout(2000);
    result = await pollingUrlRequest(
      dispatch,
      newFileId,
      convertType,
      componentType,
    ...
    fileUrls.some(
      file => file.convertStatus === 'failed' || file.convertStatus === 'stopped'
    )
  ) {
 ...
 */
export function uploadCroppedImage(file, payload, basicId) {
  const {
    contentType,
    fileId,
    fileName,
  ...
  return async (dispatch) => {
    ...
    if (!newFileId) {
      ...
      console.log('getting uploadToS3 - file: ', file);
      console.log(
        'getting uploadToS3 - signature.payload.response: ',
    ...
    const result = await pollingUrlRequest(
      dispatch,
      newFileId,
      convertType,
      componentType,
