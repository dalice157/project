...
exports.app = (app) => {
  ...
  app.use('/50talent/_api', (req, res, next) => {
    ...
    if (req.headers.host && ['localhost:8080', 'localhost:8083', '127.0.0.1:8080', '127.0.0.1:8083'].includes(req.headers.host)) {
      next()
    } else if (!req.headers.referer || (req.headers.referer && req.headers.referer === `https://${process.env.SENIOR_DOMAIN}`)) {
      next()
    } else {
      res.status(403)
