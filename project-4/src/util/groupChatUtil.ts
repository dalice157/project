...
export const sendGroupMsg = ({
    ...
    textPlugin.data({
        ...
        error: function (reason: any) {
            console.log("群聊訊息發送失敗 原因:", reason);
        },
        success: function () {
            console.log("群聊訊息發送成功");
        },
    });
};

/*textPlugin ondata 接收到的事件處理方法 */
export const groupChatProcessDataEvent = (data: any, eventKey: any) => {
    const chatStore = useChatStore();
    const { scrollToBottom } = chatStore;
    const { messages, pictures, findScrollHeight, newMsgHint, isOnline, participantList, isCallJoin } =
    ...
    const whatStatus: whatType = {
        ...
        join: () => {
            ...
            if (participantList.value.length > 0) {
                isOnline.value = true;
                isCallJoin.value = true;
            }
            console.log("有人加入後 房間的人員清單", participantList.value);
        },
        leave: () => {
            console.log("有人離開房間aaa->", data);
            if (participantList.value.length === 0) {
