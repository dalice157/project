...
const { getHistoryMessage } = apiStore;
const { historyMessageListFirst, historyMessageList } = storeToRefs(apiStore);
...
//首次進入打歷史訊息API
onMounted(() => {
    getHistoryMessage(route.params.eventKey, false);
});
// 每五秒刷一次歷史訊息API
const timer = ref(null);
timer.value = setInterval(() => {
    getHistoryMessage(route.params.eventKey, true);
}, 5000);
...
onMounted(() => {
    // 初始化陣列
    watch(
        historyMessageListFirst,
        () => {
            console.log("api初始化陣列", historyMessageListFirst.value);
            ...
            if (localStorage.getItem("recordList") === null) {
                ...
                historyMessageListFirst.value.forEach((item) => {
                    ...
                    if (localDeviceHasChatroom.includes(item.chatToken)) {
                        ...
                        recordList.value.push(roomObj);
                    }
                });
                changeList.value = recordList.value;
                recordMessages.value = recordList.value;
                localStorage.setItem("recordList", JSON.stringify(recordList.value));
                console.log("首次創建recordList", recordList.value);
            } else {
                //如果有recordlist的狀況
                ...
                // console.log("初始化聊天室有的歷史紀錄", recordList.value);
                const localDeviceHasChatroom = JSON.parse(
                    localStorage.getItem("localDeviceHasChatroom")
                );
                // console.log("本地裝置有的聊天室", localDeviceHasChatroom);
                // console.log(" 之前有創過的 recordList ", recordList.value);
                // recordList 有的room
                const recordHasRoom = recordList.value.map((item) => item.chatToken);
                // console.log("之前 recordList 創過的 room", recordHasRoom);
                const addRoom = localDeviceHasChatroom.filter((item) => {
                    return recordHasRoom.indexOf(item) === -1;
                });
                // console.log("需要添加的room", addRoom);
                const apiHasRoom = historyMessageListFirst.value.map((item) => item.chatToken);
                ...
                recordList.value.sort((a, b) => b.janusMsg.time - a.janusMsg.time);
                const toTopLists = recordList.value.filter((item) => {
                    return item.toTop;
                });
                const doNotTopLists = recordList.value.filter((item) => {
                    return !item.toTop;
                });
                // .sort((a, b) => a.key - b.key);
                recordList.value = [...toTopLists, ...doNotTopLists];
                console.log("初始化更新過後的local資料", recordList.value);
                changeList.value = recordList.value;
                recordMessages.value = recordList.value;
                localStorage.setItem("recordList", JSON.stringify(recordList.value));
            }
    ...
    watch(
        historyMessageList,
        () => {
            console.log("每5秒刷新api 更新訊息", historyMessageList.value);
            recordList.value = JSON.parse(localStorage.getItem("recordList"));
            // console.log("聊天室有的歷史紀錄", recordList.value);
            // 若local有資料更新最後一筆訊息
            recordList.value.forEach((record) => {
                historyMessageList.value.forEach((item) => {
                    //更新狀態
                    if (record.chatToken === item.chatToken) {
                        record.name = item.name;
                        record.icon = item.icon;
                        record.description = item.description;
                        record.homeurl = item.homeurl;
                        //若有訊息更新訊息
                        if (item.janusMsg.config !== null) {
                            record.janusMsg = item.janusMsg;
                            record.unread = true;
                        }
                    }
                });
...
const openFunctionPopUp = (num: any) => {
    ...
    localStorage.setItem(`recordList`, JSON.stringify(changeList.value));
};
// //刪除功能
